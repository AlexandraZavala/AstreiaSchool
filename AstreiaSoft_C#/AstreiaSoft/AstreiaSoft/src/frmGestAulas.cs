    ...
    public partial class frmGestAulas : Form
    {
        ...
        private void btnGuardar_Click(object sender, EventArgs e)
        {
            ...
            foreach (Control control in panelSecciones.Controls)
            {
                ...
                {
                    _seccion = formSec.GetSeccion(); 
                    _seccion.activo = true;
                    break;
                }
            }
            if (_estado == Estado.Nuevo)
            {
                resultadoSeccion = _daoInfraWS.modificarSeccion(_seccion);
                resultadoAula = _daoInfraWS.insertarAula(_aula);
                _aula.idAula = resultadoAula;
            }

            if (_estado == Estado.Editar)
            {
                resultadoAula = _daoInfraWS.modificarAula(_aula);
            }
            if (_seccion != null)
            {
                resultadoSeccion = _daoInfraWS.asignarAulaXSeccion(_seccion.idSeccion, _aula.idAula);
                _seccion.aula = _aula;
            }
        ...
        {
            var resultado = _daoInfraWS.listarAulasDetalle(txtNombreBusqAula.Text);

            // Verificar si el resultado no es null antes de convertirlo a lista
            if (resultado != null)
            {
                dgvAulas.DataSource = resultado.ToList();
            }
            else
            {
                // Manejar la situación en que el resultado es null
                // Por ejemplo, puedes limpiar el DataSource o mostrar un mensaje
        ...
        {
            // Limpia los campos de texto o cualquier otro control que almacene datos de entrada.
            txtNombreAula.Text = "";
            txtCodigoAula.Text = "";
            txtCapacidadAula.Text = "";
            txtIdAula.Text = ""; // Si es necesario limpiar también el ID
            
            // Restablece el objeto _aula si es necesario
            _aula = new InfraWS.aula(); // Suponiendo que Aula es una clase y que puedes instanciarla así.
        }
