...
public class CursoProgramadoMySQL implements CursoProgramadoDAO{
    ...
    @Override
    public int eliminarHorarios(int idCursoProgramado) {
        int resultado = 0;
        try{
            con = DBManager.getInstance().getConnection();
            cs = con.prepareCall("call ELIMINAR_HORARIOS_POR_CURSO_PROGRAMADO(?)");
            cs.setInt("_id_curso_programado", idCursoProgramado);
            resultado = cs.executeUpdate();
        }catch(Exception ex){
            System.out.println(ex.getMessage());
    ...
    public ArrayList<CursoProgramado> listarTodos() {
        ...
        try{
            ...
            while(rs.next()){
                ...
                cursoProgramado.setNombreCursoGrado(rs.getString("nombre_completo"));
    ...
    @Override
    public int verificar_docente(int idCursoP, int idDocente){
        int resultado = 0;
        try{
            con = DBManager.getInstance().getConnection();
            cs = con.prepareCall("{call VERIFICAR_DOCENTE_CURSOPROGRMADO(?,?)}");
            cs.setInt("p_idCursoProgramado", idCursoP);
    ...
    @Override
    public int asignarDocenteCursoProgramado(int idCursoProgramado, int idDocente){
        int resultado = 0;
        try{
            con = DBManager.getInstance().getConnection();
            cs = con.prepareCall("{ call INSERTAR_CURSO_PROGRAMADO_DOCENTE(?, ?) }");
            cs.setInt(1, idCursoProgramado);
